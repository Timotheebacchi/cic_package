  ...
  The panel-data workflow uses `panel_data = TRUE` in `sim_dgp()`,
  `check_cic_assumptions()`, and `cic()`. In that mode, the package treats
  `(Y, Z)` as a paired sample, switches the variance scaling to
  $N = \min(n_1, n_2)$, and currently supports the `no-split` method only.

  ### Panel-data assumptions

  When `panel_data = TRUE`, the user is saying that the data come from the
  panel-style setup used by the new variance estimator. In plain language, that
  means:

  - `(Y_i, Z_i)` should be observed together for the same units, so `Y` and `Z`
    are treated as a paired sample;
  - `X` is a separate i.i.d. sample, independent of the paired `(Y, Z)` data;
  - the two sample sizes should both be large enough for the split-sample
    construction to be stable;
  - the panel diagnostic is a screening rule, so a warning means the panel
    approximation is fragile, not that the CiC model is mathematically false.

  In this mode, the function uses `N = min(n_1, n_2)` for the variance scale
  and the diagnostics focus on whether the paired sample looks compatible with
  the split construction used by the estimator.
